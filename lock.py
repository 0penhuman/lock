...
# print(key('hello darkness my old friend'))
def sqrt(x,prec):# 实验证明，x的大小对运行时间几乎没有影响，prec的大小则有显著影响;用时与prec的关系非线性，斜率逐渐增大
...
def coding(word,key):
    ...
    for i in range(len(word)):
        # print(ord(word[i])+int(sqrt(key,prec)[i]),end=',')
        encode.append(ord(word[i])+int(sqrt(key,prec)[i]))
    return encode
print(coding('hello',key('j')))
def decode(word,key):
    prec = len(word)
    encode=[]
    for i in range(len(word)):
...
#     for i in range(0,5):
#         file.write(numtohex(coding('hello',key('j'))[i]))
while 1:
    command=input('-->')
    ...
    if command=='decode':
        ...
        for i in range(1,int(len(word)/6+1)):
            spilt.append(chr(int(word[(i-1)*6:i*6],16)))    #分割文件内容，6个一组
        spilt=''.join(spilt)
